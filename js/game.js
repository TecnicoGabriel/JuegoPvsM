...
function loadLeaderboard() {
    ...
    db.ref('scores').orderByChild('score').limitToLast(5).on('value', (snapshot) => {
        ...
        if (data.length === 0) {
            ...
            // Creamos la lista de los 5 primeros
...
// Generador de nombres de pistas track1.mp3 a track10.mp3
...
const images = {
    p1: img('jugador.png'), 
    p2: img('jugador2.png'),
    enemy: img('enemigo_base.png'), 
    shooter: img('enemigo_shooter.png'),
    tri: img('enemigo_tri.png'), 
    laser: img('enemigo_laser.png'),
    boss1: img('boss1.png'), 
    boss2: img('boss2.png'), 
    boss3: img('boss3.png'), 
    bossF: img('boss4.png'),
    bulletP: img('corazon.png'), 
    bulletE: img('bala_enemiga.png'), 
    bulletC: img('bala_canon.png'),
    helper: img('ayudante.png'),
    pMetra: img('poder_metralleta.png'), 
    pCanon: img('poder_canon.png'),
    pAyuda: img('poder_ayudante.png'), 
    pVida: img('poder_vida_full.png'), 
    pVidaSmall: img('poder_vida.png'),
    pSpread: img('poder_spread.png'), 
    pDoble: img('poder_arma.png')
};
...
class Player {
    ...
    update() {
        ...
        // Movimiento Táctil (Solo P1)
        if(this.id === 1 && isTouching && touchX) {
            this.x = touchX - this.w / 2; 
            this.y = touchY - this.h / 2;
            if(Date.now() - this.lastShot > this.cadence) { 
                this.shoot(); 
                this.lastShot = Date.now(); 
            }
        }
        
        if(keys[this.ctrl.shoot] && Date.now() - this.lastShot > this.cadence) { 
            this.shoot(); 
            this.lastShot = Date.now(); 
        }
    ...
    shoot() {
        ...
        if(this.weapon === 'normal') {
            ...
            for(let i=0; i<4; i++) 
                projectiles.push({x:this.x+(i*15), y:this.y, vx:0, vy:-v, w:10, h:10, img:images.bulletP, dmg:dmg});
        }
    ...
    draw(){ 
        ...
        // Barra de vida pequeña debajo del jugador
...
class Helper {
    ...
    update(){
        ...
        if(this.p.hp > 0){
            ...
            this.y = this.p.y + 20;
            if(frames % 40 === 0) 
                projectiles.push({x:this.x, y:this.y, vx:0, vy:-7, w:10, h:10, img:images.bulletP, dmg:5});
        }
    }
    draw(){
        ctx.globalAlpha = 1; 
        ctx.drawImage(images.helper, this.x, this.y, this.w, this.h);
    }
}
    ...
    update() {
        ...
        if(this.type === 'boss') {
            ...
            if(frames % fireRate === 0) {
                ...
                if(hpPerc > 0.7) { 
                    ...
                    if(this.bossType >= 3) {
                        for(let i=0; i<8; i++){
                            let a = (Math.PI*2/8) * i; 
                            enemyProjectiles.push({x:cx,y:cy+20,vx:Math.cos(a)*5,vy:Math.sin(a)*5,w:bS,h:bS});
                        }
                    } else {
            ...
            if(frames % 120 === 0) {
                ...
                if(this.type === 'tri') { 
                    enemyProjectiles.push({x:cx,y:cy,vx:0,vy:5,w:15,h:15}); 
                    enemyProjectiles.push({x:cx,y:cy,vx:-2,vy:5,w:15,h:15}); 
                    enemyProjectiles.push({x:cx,y:cy,vx:2,vy:5,w:15,h:15}); 
                }
...
window.iniciarJuego = () => {
    ...
    document.getElementById('gameOverScreen').classList.add('hidden'); // Asegurar que Game Over esté oculto
...
function loop() {
    ...
    players.forEach(p => { 
        ...
        p.draw(); 
        if(p.hp > 0) alive++; 
        document.getElementById(`p${p.id}Stats`).innerText = `P${p.id}: ${Math.floor(p.hp)}%`;
    });
